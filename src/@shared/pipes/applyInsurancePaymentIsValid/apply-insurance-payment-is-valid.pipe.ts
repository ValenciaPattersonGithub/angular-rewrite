...
@Pipe({ name: '[ApplyInsurancePaymentIsValid]' })
export class ApplyInsurancePaymentIsValidPipe implements PipeTransform {
  transform(claimsList: ClaimEntity[], canEditAllowedAmount: boolean): boolean {    
    // validate that at least one claim has a value or is being closed
        ...
        // if canEditAllowedAmount is true and the service is on a fee schedule or is a new fee schedule item
