function BaseSchedulerCtrl($scope, className) {
  ...
  function cleanupScope(event) {
    if (event && event.currentScope) {
      ...
      //    if (scopeItem && event.currentScope.hasOwnProperty(scopeItem) && !scopeItem.startsWith('$')) {
      //        event.currentScope[scopeItem] = null;
      //    }
      //}

      //if (event.currentScope.$$watchers && event.currentScope.$$watchers.length) {
      ...
      //if (event.currentScope.$$listeners && event.currentScope.$$listeners.$destroy) {
      //    delete event.currentScope.$$listeners.$destroy;
      //}
      //if (event.currentScope.$$listenerCount && event.currentScope.$$listenerCount.$destroy) {
      ...
      // cleanupListeners(event.currentScope.$$listeners, event.currentScope.$$listenerCount)
    }
  }
