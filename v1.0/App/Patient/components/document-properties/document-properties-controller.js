'use strict';
angular.module('Soar.Patient').controller('DocumentPropertiesController', [
  '$scope',
  'localize',
  'PatientServices',
  'toastrFactory',
  ...
  function (
    ...
    ctrl.updateDocument = function () {
      $scope.document.Name = $scope.document.Name.trim().replace(/ /g, "_");
    ...
    ctrl.validateDocument = function () {
      ...
      $scope.documentNameBlur = function () {
        if ($scope.document.Name !== null) {
            $scope.document.Name = $scope.document.Name.trim().replace(/ /g, "_");
        }
    }
