...
angular.module('Soar.Patient').factory('PreventiveCareGridFactory', [
  '$resource',
  '$filter',
  'localize',
  'GridOptionsFactory',
  ...
  function ($resource, $filter, localize, gridOptionsFactory, timeZoneFactory) {
    ...
    options.id = 'PreventiveCareGrid';

    // Grid Definition
    options.query = $resource(
      '_soarapi_/patients/PreventiveCarePlans',
      {},
      { getData: { method: 'POST' } }
    );
          ...
          function (data, row) {
            if (row['IsActive'] === false) {
              ...
              return (
                ...
                _.escape(tiptitle) +
                '">&nbsp;</span><div ng-click="actions.navToPatientProfile(\'' +
                row['PatientId'] +
                '\')">' +
                _.escape(data) +
                '</div>'
