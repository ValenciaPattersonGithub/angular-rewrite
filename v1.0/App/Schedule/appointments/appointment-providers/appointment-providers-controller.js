...
app.controller('AppointmentProvidersController', [
  '$scope',
  '$timeout',
  'ListHelper',
  'TimeZoneFactory',
  function (
      ...
      _.forEach(
        ...
        function (providerAppointment) {
          ...
          let startTime = timeZoneFactory.ConvertDateToMomentTZ(
            startTimeToUse,
            $scope.appointment.Location.Timezone
          );
          ...
          for (let i = 0; i < $scope.slots.length; i++) {
            let slot = $scope.slots[i];
            if (
              slot.Start.getHours() === startTime.hours() &&
              slot.Start.getMinutes() === startTime.minutes() &&
              slot.End.getHours() === endTime.hours() &&
              slot.End.getMinutes() === endTime.minutes()
            ) {
              providerSchedules.push({
    ...
    $scope.checkProviderMissingHours = function () {
      $scope.providerValidationMissingHours = [];
      if ($scope.appointment.Classification == 0) {
    ...
    $scope.$watch(
      ...
      function (nv, ov) {
        if (nv && nv !== ov) {
          ...
          ctrl.filterProviderConflicts();
          ctrl.filterOutsideWorkingHoursConflicts();
          // if selected provider has changed and there is not provider schedules
          // we need to add providerSchedules
          if (
            $scope.providerSchedules &&
