//describe('schedule-appointment-status directive controller ->', function () {
...
//	var scope, ctrl, mockStaticData, mockListHelper, timeout, patientValidationFactory, patientAppointmentsFactory, location, localize, patientServices, window;
//	var modalFactory, locationService;
...
//	    ScheduleAppointmentStatusModal: jasmine.createSpy().and.returnValue({
...
//		localize = {
//            getLocalizedString:jasmine.createSpy().and.callFake(function (val) { return val; })
//        };
...
//		mockStaticData = {
//			AppointmentStatuses: jasmine.createSpy().and.returnValue(angular.copy(mockStatusData))
//		};

//		mockListHelper = {
//			findItemByFieldValue: jasmine.createSpy().and.returnValue(angular.copy(mockStatusList[1]))
//		};

//		ctrl = $controller('ScheduleAppointmentStatusController', {
//			$scope: scope,
...
//			expect(scope.$watch).toHaveBeenCalledWith('appointment.Status', ctrl.appointmentStatusChanged);
//		});
//	});
...
//			expect(mockListHelper.findItemByFieldValue).toHaveBeenCalledWith(scope.statusList, 'Value', 'Z');
...
//    		mockListHelper.findIndexByPredicate = jasmine.createSpy().and.returnValue(-1);
//    	});
...
//    		mockListHelper.findIndexByPredicate = jasmine.createSpy().and.callFake(function (value) {
...
//			expect(ctrl.disableOption).toHaveBeenCalledWith(scope.statuses.InReception);
...
//			scope.appointment = { Status: scope.statuses.Completed, StartTime: today };

//			ctrl.disableOption = jasmine.createSpy();

//			ctrl.disableControls();

//			expect(ctrl.disableOption).not.toHaveBeenCalledWith(scope.statuses.InReception);
//		});
//	});
...
//	    beforeEach(function () {
//	        mockListHelper.findIndexByFieldValue = jasmine.createSpy().and.returnValue(1);
//	        spyOn(angular, 'element').and.returnValue(['first option item', 'second option item', 'third option item']);
//	    });
...
//			expect(mockListHelper.findIndexByFieldValue).toHaveBeenCalledWith(scope.statusList, 'Value', 'Z');
...
//            expect(scope.onChange).toHaveBeenCalledWith(scope.appointment, false, true, null, null, false);
//        });
...
//			timeout.flush();
//            expect(scope.onChange).toHaveBeenCalledWith(scope.appointment, true, false, null, null, false);
//        });
...
//			timeout.flush();
//            expect(scope.onChange).toHaveBeenCalledWith(scope.appointment, true, false, null, null, true);
//        });
...
//			scope.selectedStatusChanged({}, { Value: 10 }, false);
//			timeout.flush();
//            expect(scope.onChange).toHaveBeenCalledWith(scope.appointment, false, true, null, null, true);
//        });
...
//			locationService.getCurrentLocation = jasmine.createSpy().and.returnValue(loggedInLocation);

//			scope.appointment = {};

//			mockModalFactory.LocationChangeForStartAppointmentModal = jasmine.createSpy().and.returnValue({ then: function () { } });
...
//				expect(mockModalFactory.LocationChangeForStartAppointmentModal).toHaveBeenCalled();
//			});

//			describe('modalFactory success callback ->', function () {

//				beforeEach(function () {
//					mockModalFactory.LocationChangeForStartAppointmentModal.and.returnValue({ then: function (success) { success(); } });
//				});

//				it('should call ctrl.updateAppointmentStatusForStart with correct parameters', function () {
//					scope.startAppointment();

//					expect(ctrl.updateAppointmentStatusForStart).toHaveBeenCalledWith(true);
//				});
...
//				beforeEach(function () {
//					result = 'result';
//					mockModalFactory.LocationChangeForStartAppointmentModal.and.returnValue({ then: function (success, failure) { failure(result); } });
//				});
...
//			var result = 'result';
//			beforeEach(function () {
//				scheduleServices.AppointmentStatus.Update = function (data, success) { success(result); };
...
//			expect(beginAppointmentSpy).toHaveBeenCalledWith(jasmine.anything(), result.Value);
//			expect(startAppointmentSpy).toHaveBeenCalledWith(jasmine.anything(), result.Value);
//		});
...
//			expect(scope.PreviousLocationRoute).toBe(`#/Patient/${result.Value.PersonId}/Clinical/?activeSubTab=0`);
//		});
...
//			expect(scope.PreviousLocationRoute).toBe(`#/Patient/${result.Value.PersonId}/Clinical/?activeSubTab=0&setLocation=${result.Value.LocationId}`);
//		});
...
//			expect(window.open).toHaveBeenCalledWith(scope.PreviousLocationRoute, '_blank');
//		});
...
//		beforeEach(function () {
//			scope.appointment = { Status: 'status', originalStatus: { Value: 'originalStatus' } };
//		});

//		it('should set values', function () {
//			scope.disableControl = true;
//			scope.afterBeginFailed();
//			expect(scope.appointment.Status).toBe(scope.appointment.originalStatus.Value);
...
//			ctrl.getOptionItem = jasmine.createSpy().and.returnValue({ disabled: false });
//        });
...
//			timeout.flush();
//            expect(scope.onChange).toHaveBeenCalledWith(scope.appointment, false, false, null, null, true);
//        });
...
//			scope.selectedStatusChange('5');
//			timeout.flush();
//            expect(scope.onChange).toHaveBeenCalledWith(scope.appointment, false, false, null, null, false);
//        });
...
//		var appt = { AppointmentId: 'appointmentId', Patient: { PatientId: 'patientId' } };
//		var search = 'search';
//		beforeEach(function () {
...
//				expect(ctrl.showAppointmentSaveModal).toHaveBeenCalledWith(appt, scope.patient);
//			});
...
//						Appointments: {
//							GetEncounterIdForAppointment: jasmine.createSpy().and.callFake(
//								function (param, success, failure) {
//									if (successFlag === true)
//										success(mockResult);
//									else
//										failure();
...
//					ctrl.canCheckout(appt);
//					expect(scheduleServices.Lists.Appointments.GetEncounterIdForAppointment).toHaveBeenCalledWith({ AppointmentId: appt.AppointmentId }, jasmine.any(Function), jasmine.any(Function));
//				});
...
//					ctrl.canCheckout(appt);
//					expect(ctrl.sendForCheckout).toHaveBeenCalledWith(value.EncounterId, appt);
//				});
...
//					mockResult = { Value: value };
//					ctrl.canCheckout(appt);
//					expect(ctrl.showAppointmentCheckedOutMessage).toHaveBeenCalledWith(appt);
//				});
...
//		var appt;
//		beforeEach(function () {
//			mockModalFactory.ConfirmModal = jasmine.createSpy().and.returnValue({ then: function () { } });
//			appt = 'appointment';
//			scope.statuses = {};
//		});
...
//			expect(mockModalFactory.ConfirmModal).toHaveBeenCalledWith(jasmine.any(String), jasmine.any(String), jasmine.any(String));
//		});

//		describe('modalFactory.ConfirmModal callback function ->', function () {

//			beforeEach(function () {
//				mockModalFactory.ConfirmModal.and.returnValue({ then: function (cb) { cb(); } });
...
//				expect(patientAppointmentsFactory.setLoadHistory).toHaveBeenCalledWith(true);
...
//	describe('ctrl.sendForCheckout function ->', function () {

//		var url = 'url';
//		beforeEach(function () {
//			window.open = jasmine.createSpy();
//			location.absUrl = jasmine.createSpy().and.returnValue({ replace: function () { return url; } });
//			scope.sentForCheckout = false;
//		});

//		describe('when encounterId is not null ->', function () {

//			it('should set sentForCheckout and call open with correct path if patient and person account are not null', function () {
//				var patientId = 'patientId';
//				var accountId = 'accountId';
//				var encounterId = 'encounterId';

//				ctrl.sendForCheckout(encounterId, { Patient: { PatientId: patientId, PersonAccount: { AccountId: accountId } } });

//				expect(scope.sentForCheckout).toBe(true);
//				expect(location.absUrl).toHaveBeenCalled();
//				expect(window.open).toHaveBeenCalledWith(`${url}/Patient/${patientId}/Account/${accountId}/Encounter/${encounterId}/Checkout/AccountSummaryEncounter`, '_blank');
//			});

//			it('should not set sentForCheckout nor call open if patient is null', function () {
//				var encounterId = 'encounterId';

//				ctrl.sendForCheckout(encounterId, {});

//				expect(scope.sentForCheckout).toBe(false);
//				expect(window.open).not.toHaveBeenCalled();
//			});

//			it('should not set sentForCheckout nor call open if PersonAccount is null', function () {
//				var encounterId = 'encounterId';

//				ctrl.sendForCheckout(encounterId, { Patient: {} });
...
//		describe('when encounterId is null ->', function () {

//			beforeEach(function () {
//				location.search = jasmine.createSpy().and.returnValue({ 'key1': 'value1', 'key2': 'value2' });
//			});

//			it('should set sentForCheckout and call open with correct path', function () {
//				var patientId = 'patientId';
//				var accountId = 'accountId';
//				var appointmentId = 'appointmentId';

//				ctrl.sendForCheckout(null, { AppointmentId: appointmentId, Patient: { PatientId: patientId, PersonAccount: { AccountId: accountId } } });

//				expect(scope.sentForCheckout).toBe(true);
//				expect(location.absUrl).toHaveBeenCalled();

//				expect(window.open).toHaveBeenCalledWith(`${url}/Patient/${patientId}/Account/${accountId}/Encounters/Schedule?key1=value1&key2=value2&appt=${appointmentId}`, '_blank');
//			});
...
//			patientAppointmentsFactory.PatientHasRunningAppointment = jasmine.createSpy().and.returnValue({
//                then: jasmine.createSpy().and.returnValue({Value:{HasRunningAppointment:true}})
//            });
//        });

//        it('should call ctrl.setHasRunningAppointments if no appointments are passed to directive', function () {
//            ctrl.setHasRunningAppointments();
//            expect(patientAppointmentsFactory.PatientHasRunningAppointment).toHaveBeenCalledWith(scope.person.PatientId);
//        });
...
//            ctrl.disableControls();
//            expect(ctrl.disableOption).toHaveBeenCalledWith(scope.statuses.StartAppointment);
//		});
//	});
...
//        beforeEach(function(){
//			appointmentParam ={Status:'0', ActualStartTime:null, Patient:{PatientId:'3456'}};
//			scope.appointments =  [
...
//			scope.appointments.push({Status:'4', ActualStartTime:'2018-08-28 20:54:12.2865967', Patient:{PatientId:'3456'}});
