describe('fee-schedule-adjustment-controller', function () {
  ...
  beforeEach(inject(function ($rootScope, $controller, $q) {
    ...
    scope.data = {
      ...
      Title: 'Fee Schedule Adjustments',
      Locations: [{ 0: 1 }],
    };
    ...
    factory = {
      GetFeeScheduleAdjustment: jasmine.createSpy().and.returnValue({
        then: jasmine
          .createSpy()
    ...
    ctrl = $controller('FeeScheduleAdjustmentController', {
      $scope: scope,
  ...
  describe('processInitMode function ->', function () {
    it('processInitMode should be called', function () {
      ...
      expect(factory.GetFeeScheduleAdjustment).toHaveBeenCalledWith(
        filterResult
      );
    });
    ...
    it('processInitMode should be called', function () {
      var initMode = 2;
      ctrl.processInitMode(initMode);
      expect(scope.feeScheduleAdjustmentModel).toEqual(scope.data.initData);
      expect(scope.feeScheduleAdjustmentModel.title).toEqual(
        'Fee Schedule Adjustments'
      );
      expect(scope.feeScheduleAdjustmentModel.isCurrency).toBe(true);
      expect(scope.feeScheduleAdjustmentModel.barStyle).toEqual(0);
    });
  });
