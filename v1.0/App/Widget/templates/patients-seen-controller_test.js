describe('patients-seen-controller', function () {
  ...
  beforeEach(inject(function ($rootScope, $controller, $q) {
    ...
    factory = {
      GetPatientsSeen: jasmine.createSpy().and.returnValue({
        then: jasmine
          .createSpy()
    ...
    ctrl = $controller('PatientsSeenController', {
      $scope: scope,
  ...
  describe('processInitMode function ->', function () {
    it('processInitMode should be called', function () {
      ...
      expect(factory.GetPatientsSeen).toHaveBeenCalledWith(filterResult);
    });

    it('processInitMode should be called', function () {
      var initMode = 2;
      ctrl.processInitMode(initMode);
      expect(scope.patientsSeenModel).toEqual(scope.data.initData);
      expect(scope.patientsSeenModel.title).toEqual('Patients Seen');
      expect(scope.patientsSeenModel.isCurrency).toBe(false);
      expect(scope.patientsSeenModel.barStyle).toEqual(1);
      expect(scope.patientsSeenModel.reportId).toEqual(14);
    });
  });
