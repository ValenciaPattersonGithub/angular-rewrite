describe('new-patients-controller', function () {
  ...
  beforeEach(inject(function ($rootScope, $controller, $q) {
    ...
    factory = {
      GetNewPatients: jasmine.createSpy().and.returnValue({
        then: jasmine
          .createSpy()
    ...
    ctrl = $controller('NewPatientsController', {
      $scope: scope,
  ...
  describe('processInitMode function ->', function () {
    it('processInitMode should be called', function () {
      ...
      expect(factory.GetNewPatients).toHaveBeenCalledWith(filterResult);
    });

    it('processInitMode should be called', function () {
      var initMode = 2;
      ctrl.processInitMode(initMode);
      expect(scope.newPatientsModel).toEqual(scope.data.initData);
      expect(scope.newPatientsModel.title).toEqual('New Patients');
      expect(scope.newPatientsModel.isCurrency).toBe(false);
      expect(scope.newPatientsModel.barStyle).toEqual(1);
      expect(scope.newPatientsModel.reportId).toEqual(16);
    });
  });
