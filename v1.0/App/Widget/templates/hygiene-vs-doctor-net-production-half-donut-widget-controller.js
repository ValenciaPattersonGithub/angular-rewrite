...
angular
  .module('Soar.Widget')
  .controller('HygieneVsDoctorNetProductionHalfDonutWidgetController', [
    '$scope',
    'WidgetInitStatus',
    'WidgetFactory',
    'ReportsFactory',
...
function HygieneVsDoctorNetProductionHalfDonutWidgetController(
  $scope,
  widgetInitStatus,
  widgetFactory,
  reportsFactory,
  ...
  $scope.hygieneVsDoctorNetProductionCharts = [];
  $scope.hygieneVsDoctorNetProductionSeriesData = [];
  $scope.hygieneVsDoctorNetProductionFilters = [];
  $scope.hygieneVsDoctorNetProductionCurrentFilter = null;
  ...
  $scope.hygieneVsDoctorNetProductionChangeFilter = function (filter) {
    if (!$scope.isLoadingForFilter && !_.isNull(filter)) {
      $scope.hygieneVsDoctorNetProductionCurrentFilter = filter;
      ctrl.getDataFromServer();
    }
  };
  ...
  ctrl.getDataFromServer = function () {
    ...
    widgetFactory.GetHygieneVsDoctorNetProduction(filterResult).then(
      function (res) {
  ...
  ctrl.processDataFromServer = function (data) {
    $scope.hygieneVsDoctorNetProductionCharts = data.Data;
    $scope.dateFilter = Object.keys(data.Data)[0];
    if (!_.isNil($scope.data) && !_.isNil($scope.data.Locations)) {
      $scope.locations = $scope.data.Locations;
    }

    _.each($scope.hygieneVsDoctorNetProductionCharts, function (chart, key) {
    ...
    $scope.hygieneVsDoctorNetProductionFilters = filters;
    $timeout(function () {
      if (_.isNull($scope.hygieneVsDoctorNetProductionCurrentFilter)) {
        $scope.hygieneVsDoctorNetProductionCurrentFilter = data.DefaultFilter;
      }
      $scope.hygieneVsDoctorNetProductionSeriesData =
        $scope.hygieneVsDoctorNetProductionCharts[
          $scope.hygieneVsDoctorNetProductionCurrentFilter
...
HygieneVsDoctorNetProductionHalfDonutWidgetController.prototype = Object.create(
  BaseCtrl.prototype
);
