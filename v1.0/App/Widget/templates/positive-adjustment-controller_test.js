describe('positive-adjustment-controller', function () {
  ...
  beforeEach(inject(function ($rootScope, $controller, $q) {
    ...
    factory = {
      GetPositiveAdjustment: jasmine.createSpy().and.returnValue({
        then: jasmine
          .createSpy()
    ...
    ctrl = $controller('PositiveAdjustmentController', {
      $scope: scope,
  ...
  describe('processInitMode function ->', function () {
    it('processInitMode should be called', function () {
      ...
      expect(factory.GetPositiveAdjustment).toHaveBeenCalledWith(filterResult);
    });

    it('processInitMode should be called', function () {
      var initMode = 2;
      ctrl.processInitMode(initMode);
      expect(scope.positiveAdjustmentModel).toEqual(scope.data.initData);
      expect(scope.positiveAdjustmentModel.title).toEqual(
        'Positive Adjustments'
      );
      expect(scope.positiveAdjustmentModel.isCurrency).toBe(true);
      expect(scope.positiveAdjustmentModel.barStyle).toEqual(0);
      expect(scope.positiveAdjustmentModel.reportId).toEqual(22);
      expect(scope.positiveAdjustmentModel.adjustmentTypeFieldName).toEqual(
